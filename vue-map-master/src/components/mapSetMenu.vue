...
export default {
  ...
  data() {
    return {
      ...
      value1: 0,
      menuList: [
        { id: 1, icon: "ios-analytics", title: "关闭工具栏", color: "#cb5a5e", page: DataMonitor },
        { id: 2, icon: "md-apps", title: "显示监控卡口", color: "#30962d", page: test2 },
        { id: 3, icon: "md-aperture", title: "显示轨迹", color: "#ca8319", page: test2 },
        { id: 4, icon: "md-analytics", title: "3D地图", color: "#ed3f14", page: test2 },
      ]
    };
  },
  filters: {
  ...
  methods: {
    ...
    1() {
      if (this.menuList[0].title === "显示工具栏") {
        this.$parent.$refs.mapView.visible.control = true
        this.$set(this.menuList[0], 'title', "关闭工具栏")
      } else {
        this.$parent.$refs.mapView.visible.control = false
        this.$set(this.menuList[0], 'title', "显示工具栏")
      }

    },
    2() {
      if (this.menuList[0].title === "显示监控卡口") {
        this.$parent.$refs.mapView.visible.marker = true
        this.$set(this.menuList[0], 'title', "隐藏监控卡口")
      } else {
        this.$parent.$refs.mapView.visible.marker = false
        this.$set(this.menuList[0], 'title', "显示监控卡口")
      }
    },
    3() {
      if (this.menuList[0].title === "显示轨迹") {
        this.$parent.$refs.mapView.visible.line = true
        this.$set(this.menuList[0], 'title', "隐藏轨迹")
      } else {
        this.$parent.$refs.mapView.visible.line = false
        this.$set(this.menuList[0], 'title', "显示轨迹")
      }
    },
    4() {
      if (this.menuList[0].title === "3D地图") {
        this.$parent.$refs.mapView.pitch = 55
        this.$set(this.menuList[0], 'title', "2D地图")
      } else {
        this.$parent.$refs.mapView.pitch = 0
        this.$set(this.menuList[0], 'title', "3D地图")
      }
    },
    //格式化菜单数据,以6来均分
